...
class DB:
    ...
    def order_info(self, par):
        self.cursor.execute(
            '''
            SELECT (SELECT price FROM orders WHERE product.idp = id_i) as 'price',
            (SELECT name FROM product WHERE product.idp = (SELECT id_i FROM orders WHERE product.idp = id_i)) as 'name'
